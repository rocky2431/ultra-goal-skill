...
目标和基本边界明确后，Agent 先只读发现协作能力，再询问：本目标仅使用当前宿主的子 Agent 或独立会话，还是允许向列明的其他 Agent 外派？已有明确且适用的回答就沿用。**这项确认必须早于首次派工，包括研究和规格质询。** 原始回答记入决策文件，允许的协作范围写入 `Boundary`，独立评审者和已批准替补写入 `Verification`。两种方式都保留独立最终验收；范围内日常选择 worker 不反复问，扩大范围需要你决定。详见[协作范围确认](../plugins/ultra-goal/skills/ultragoal/references/agent-modes.md#confirm-collaboration-scope)。

每一轮面谈都显示一行 `Settling:`，明确当前卡点、实质性 `owner`/`agent` 决策数、仍未解决的问题和下一问。内容按三个卡点收敛：**A 框架**（Intent、Boundary 与协作范围）、**B 执行与凭证**（其余所有 goal 段落、当前 decisions 和执行附件）、**C 整包**（规格质询后的完整包，以及最终要交付的结果与凭证包）。A/B 确认后直接更新其覆盖的实质决策行；C 是对整包的第二次确认。任何覆盖性修改都会重新打开对应的 A/B 和 C。只有 C 新增一条保留的 `owner` 行，并绑定当前冻结 goal 的 digest。validator 检查该行和摘要格式，首次 arm 再检查摘要仍与当前冻结条款一致；它能发现遗漏和过期文本，不能认证 owner 身份或判断语义充分性。当前宿主真实提供结构化提问界面时优先使用，但完整回读仍放在可读正文中；没有该界面时就问一个普通的明确问题。

Git 是运行的默认项：已有包含可用 `HEAD` 的上层仓库就直接复用；没有仓库或还没有 commit 时，Agent 先展示状态并检查可疑路径，再提议初始化和建立基线 commit，而且必须在 arm 之前完成。缺少 Git 基线时，arming fence 默认拒绝；工作区本来就不是项目、Git 不可用，或用户明确拒绝建立基线时，才使用 `--allow-no-git`，并说明将失去 step-history 覆盖及基于 commit 的 writer 排除。
...
| `Boundary` | 路径、影响、允许调用的 Agent 和审批范围；需要实测的置信声明；不能直接推断的结论 |
...
提供无人值守执行前，指令要求独立的规格质询。先给质询者原始要求、草案和证据，不先给作者的辩护。解决实质异议后执行 C 卡点的完整整包回读。已有的明确确认可以写入记录，沉默不能算确认。“直接开始”不等于放弃审查；没有实质发现的质询不改草案，C 确认的是质询后的当前整包。

包级确认行只是可审计来源记录，不是经过认证的同意。进度展示、A/B 回读质量、执行附件确认和规格质询仍主要靠**模型遵守指令**。
...
主 Agent 通过宿主的委派工具分配任务。你指定了角色，就按指定执行；其余任务由它在你确认的协作范围内判断是否需要其他工作者。小任务可以留在主会话中完成。Skill 不规定厂商顺序，也不要求每项任务都安排三家评审。
...
长期代码任务在派工前保留原始任务书、预登记攻击面与检查方法，并核对路径和工具、已集成的前置工作、实际 worktree 内的忽略资源，以及其他写入者的范围。需要脱离观察者继续运行的工作，使用原生任务句柄。具体检查及“预登记、初审、差异核对”的顺序见[角色协议](../plugins/ultra-goal/skills/ultragoal/references/agent-modes.md#before-dispatching-long-running-work)。
...
进度与回答通过所选工具真实支持的通道传递。同步桥接可能等工作者退出后才返回，这不代表它支持实时进度或会话恢复。工作者需要回答时，主 Agent 先依据已有条款解决，必要时问用户，再通过可用的回答或恢复入口继续。重试前先检查尚未确认的实际影响。Stop Hook 不提供这个通信通道。
...
已获提交授权的长期代码目标，每完成一个可检查的工作单元就提交，标题为 `goal(<slug>) step: <summary>`。正文记录 `Reason`、`Check`、`Evidence`、`Remaining`；修改产物时，再记录实际作者的 `Writer-Session`。`Evidence` 指向该次提交内保留的普通文件，路径相对于 Git 根目录。现有 `--audit` 会检查字段与原提交中的证据文件；后来补出的同名文件不能补证。完整格式见[工作提交记录](../plugins/ultra-goal/skills/ultragoal/references/document-system.md#work-step-records)。它审计已声明的步骤，不能发现所有未提交动作；旧提交格式及没有 Git 基线的目标不具备这项覆盖。
...
长期代码目标允许主 Agent 实现和派工，但最终评审者必须未参与产物实现。验收器还会排除该目标提交中用 `Writer-Session` 登记过的产物作者；遗漏身份或未汇入当前分支的工作仍需根据实际分工核对。独立评审先保存初审结论，再接收执行报告，后续差异与更正保留在初审旁边。Git 祖先关系能核对记录顺序，不能证明模型的上下文隔离或身份真实性。
...
| 症状 | 核对与处理 |
|---|---|
| 旧记录或草案出现 `OWNER_CONFIRMATION_MISSING`、`OWNER_CONFIRMATION_DUPLICATE`、`OWNER_CONFIRMATION_DIGEST_MISSING` 或 `OWNER_CONFIRMATION_STALE` | 按当前契约把它视为尚未确认；先确认 A/B，再从当前文件完整回读 C，收到 C 的明确答复后才保留一条带当前 `spec-digest` 的包级行，不能自动补写。已经 active 的旧目标可幂等恢复，但下一次 fresh arm 采用当前流程 |
| `arm` 提示没有可用 Git `HEAD` | 复用上层仓库或在获授权后建立基线 commit；用户明确接受较弱审计覆盖时，才用 `--allow-no-git` 重新 arm |
