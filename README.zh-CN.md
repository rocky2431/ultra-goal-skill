...
它以 Skill、Python 脚本和 Hook 的形式运行在 Claude Code、Codex、Kimi Code 或 zCode 中。Hook 是宿主在特定事件发生时调用的脚本，例如一个回合结束时。宿主提供模型、工具和权限控制。UltraGoal 提供目标定义和验收流程，并为 POSIX 上的 Codex 和 Claude Code 提供自主续跑与事件等待。

适合需要多轮调查、实施和评审的任务。小型的一次性工作可以直接交给 Agent 处理。

版本：2.18.0。核心脚本需要 Python 3.10 或更新版本。
...
Loop 根据反馈重复推进；Graph 明确写出任务、依赖和汇合点。两种结构都可以使用固定路由，也可以在运行时选择路由。两者共用验收和授权契约。执行能力来自宿主，UltraGoal 不附带独立的 Agent Runtime 或工作流引擎。
...
你可以直接请 Agent 查看进度或解释停止原因。手动检查状态、恢复会话和取消任务的方法见[使用指南](docs/usage.zh-CN.md)。解除 UltraGoal 绑定会取消自主执行及其待投递的等待结果。如果另外启用了原生目标，也需通过宿主控制停止它。

## 当前限制

POSIX 上的 Codex 和 Claude Code 可以使用 UltraGoal 自身的续跑与等待机制，无需开启原生 Goal；其他宿主保留既有路径。具体宿主条件和验证范围见[自主执行说明](plugins/ultra-goal/skills/ultragoal/references/autonomous-execution.md)。这需要宿主会话保持运行，不会自动重启已经关闭的应用。没有完成声明的普通 Stop 不执行 Anchor。Agent 应在最终回答前调用 `verify`，这样就能在回答中报告验收结果。
