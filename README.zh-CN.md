...
**面向 Claude Code、Codex、Kimi 和 zCode 的目标工程 Skill。** UltraGoal 将一句模糊要求整理成双方确认、可以检查的目标，再帮助宿主 Agent 根据当前证据持续推进。当前版本：**2.15.1**。

目标是不变量。Loop 在执行时决定下一步路由，Graph 提前确定路由；两者使用同一份目标契约。

UltraGoal 由 **Skill、脚本和宿主 Hook** 组成。主 Agent 选择方法和工作者，脚本检查明确的事实与证据契约，宿主提供工具、权限、会话身份、续跑和取消能力。项目没有附带 Agent Runtime、后台派工服务或图执行引擎。
...
附带脚本需要 **Python 3.10+**。运行代码和测试使用 Python 标准库。宿主需要能够调用配置中的 Python 命令，并发现对应插件组件。

### 原生插件

原生安装提供主 Skill、启动命令、评审角色和对应宿主的 Hook 配置。安装命令随宿主版本变化。下面 CC 和 Codex 的命令形式已于 2026-09-05 对照本机 CLI 帮助核对；核对帮助不等于重新完成了安装或生命周期验收。
...
Kimi 和 zCode 应使用当前版本实际提供的原生插件入口，加载 `plugins/ultra-goal`。包内提供 `kimi.plugin.json` 和 `.zcode-plugin/plugin.json`。并非每个 Kimi 发行版本都存在 `kimi plugin` 子命令。如果当前版本无法加载对应清单和 Hook，只复制 Skill 不会启用验收门。

在实际宿主中检查 Skill、启动命令和 Hook 是否被发现。避免同时启用同一套 Hook 的原生与旧版安装。清单能解析、安装提示成功，都不能证明宿主实际调用了 Hook 或执行了它的返回语义。
...
Agent 会先检查已有目标，再决定是否开始面谈。确认条款后，使用宿主暴露的 `goal-run` 命令。支持命名空间斜杠命令的宿主使用：

```text
/ultra-goal:goal-run export-ready
```

如果宿主没有暴露该命令，让它使用实际可用工具遵循包内的[启动流程](plugins/ultra-goal/commands/goal-run.md)。`.goal.md` 与配套 `.decisions.md` 必须已经存在。启动不授权 Agent 临时补造一份没人同意的目标。

无人值守还要求宿主提供实际可用、已获授权的原生续跑机制。仅把验收门绑定到目标，不能产生这个能力。

## 从一句要求到确认目标

入口先区分创建、修改、查看、执行已有目标，以及不需要目标循环的普通工作。进入具体阶段后，才读取该阶段需要的参考文件。

Init 和 Research 用于解决会改变目标定义的事实未知。Agent 自己核对仓库、工具和已有指令，只问尚未解决的实质性用户决策：**每轮一个问题**，带建议和会改变建议的条件。下面这些是需要解决的依赖，不是必须进行九轮的固定面谈。
...
**派工的是主 Agent，Hook 不派工。** 用户指定角色时按指定执行；没有指定时，只在有价值、可独立交接的任务上选择工作者。小任务可以由主会话完成。没有固定的厂商顺序，也没有每项任务必须三家评一轮的要求。
...
主 Agent 读取反馈后重写 `State`、`Lessons`、`Next`；策略改变时更新相应决策，再决定下一步。这就是反馈循环，没有 Python 调度器替模型选择策略。
...
三条经验、八条状态是压缩建议，不是有效性上限。必要的第四条经验应保留。精简摘要不能删掉某条结论唯一的依据。Hook 不会替模型补写未保存的思考，也不会自动维护全部文档。
...
测试覆盖契约校验、会话归属、候选验收、中断记账、锁、评审证据留存、宿主输出契约与包结构。2.15.1 回归通过四个 Stop 适配入口检查旧标记提示，并验证目标文件字节完全不变。这属于脚本级检查，不是四家 UI 或无人值守生命周期试验。

面谈充分性、规格质询、路由、等待工作者、状态维护和调用完成协议，仍由模型负责。脚本机械化检查明确事实，不能验证每句自然语言、鉴权共享文件身份或证明原始目标永远正确。

历史产品与宿主探针只有有限范围的证据。四家完整无人值守收口、Windows 原生生命周期、所有取消与恢复组合，以及超过 95% 的统计可靠性，仍未建立。Eval 场景定义不等于已经完成模型试验。详见[剩余验证范围](docs/wip/outstanding.md)。
